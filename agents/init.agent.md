...
3. Once a token is ready, smoke-test it with a small probe script against the LOCI endpoint (see SKILL.md Step 1b). If it is rejected, re-prompt for a fresh token before continuing.
4. Run `<plugin-dir>/setup/setup.sh` via bash — Git Bash on Windows, native bash on macOS/Linux — from the project root so all state lands in `.loci`.
